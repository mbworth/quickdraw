...
export async function main(argv = process.argv.slice(2)) {
  ...
  const cfg = {
    ...
    packetMax: flags.packetMax ?? 600, fullEvery: flags.fullEvery ?? 0, thinking: String(flags.thinking ?? 'adaptive'), effort: flags.effort ?? 'low',
  ...
  const out = await runPilot({ adapter, callModel, clock, record: rec, log: (...a) => console.error(...a), stop: stopper.signal, opts: {
    heartbeatMs: cfg.heartbeat, deadlineMarginMs: cfg.deadlineMargin, packetMax: cfg.packetMax, divisor, fullEvery: cfg.fullEvery, staleAfterMs: cfg.staleAfter,
