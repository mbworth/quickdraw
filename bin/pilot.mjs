...
export async function main(argv = process.argv.slice(2)) {
  ...
  const cfg = {
    ...
    packetMax: flags.packetMax ?? 600, fullEvery: flags.fullEvery ?? 0, overlap: flags.overlap ?? 1, eventTick: !!flags.eventTick, thinking: String(flags.thinking ?? 'adaptive'), effort: flags.effort ?? 'low', reply: flags.reply ?? 'tool', stream: !!flags.stream, reserveFor: flags.reserve ? String(flags.reserve).split(',') : [],
