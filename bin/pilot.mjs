...
const BOOLEANS = ['help', 'print-config', 'event-tick'];

export async function main(argv = process.argv.slice(2)) {
  const pre = await boot(argv, { booleans: BOOLEANS });
  const game = pre.flags.game;
  if (!game || pre.flags.help) { console.error('usage: pilot --game <name> [--model M|none] [--prompt file] [--heartbeat ms] [--deadline-margin ms] [--decision-deadline ms] [--packet-max n] [--full-every n] [--thinking adaptive|off] [--effort low|medium|high] [--max-usd x] [--max-decisions n] [--concede-on never|budget] [--record-state all|on-decision|sampled] [--run-dir dir] [--stop-file path] [--stale-after ms] [--overlap n] [--event-tick] [--<game>-* ...]'); return 64; }
  ...
  const cfg = {
    ...
    packetMax: flags.packetMax ?? 600, fullEvery: flags.fullEvery ?? 0, overlap: flags.overlap ?? 1, eventTick: !!flags.eventTick, thinking: String(flags.thinking ?? 'adaptive'), effort: flags.effort ?? 'low',
  ...
  const callModel = cfg.model === 'none' ? nullModel() : await modelFor({ adapter, model: cfg.model, system, thinking: cfg.thinking, effort: cfg.effort, decisionDeadlineMs: cfg.decisionDeadline, clock });
  const out = await runPilot({ adapter, callModel, clock, record: rec, log: (...a) => console.error(...a), stop: stopper.signal, opts: {
    heartbeatMs: cfg.heartbeat, deadlineMarginMs: cfg.deadlineMargin, packetMax: cfg.packetMax, divisor, fullEvery: cfg.fullEvery, staleAfterMs: cfg.staleAfter, maxInFlight: cfg.overlap, eventTick: cfg.eventTick,
