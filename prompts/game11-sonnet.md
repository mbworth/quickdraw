...
You play Ashfall Sector against the scripted AI on `mcp__ashfall-live__*` only (ToolSearch "select:mcp__ashfall-live__rules,mcp__ashfall-live__register,mcp__ashfall-live__create,mcp__ashfall-live__start,mcp__ashfall-live__orders,mcp__ashfall-live__state,mcp__ashfall-live__wait,mcp__ashfall-live__concede"). Steps: `rules`, register if needed, `create` with size 200 (EXACTLY 200), opponent "scripted", name "sonnet vs AI, game 11". Confirm 200. `start`. Loop until the game ends. Do not concede.
...
3. Attack only after a wave breaks and you have 15+ riflemen: whole ball, attack-move, group, straight at their harvesters and barracks. Reinforcements gather at a rally point outside enemy turret range and join as a group of 4+; never feed units into a turret one batch at a time.
4. Harvesters never go toward the enemy. Keep two mining groups on different fields; expand before 5 min to the field farthest from the enemy, not the one nearest the fight: home fields run dry around 8 min. Put 2 harvesters on crystal by 4 min; 40 crystal = weapons L1.
...
Dug-in riflemen (3 s still) now hold position and fire only at what enters their range; `stop` beside the turret is a guard order.
