...
- `P` production: `co#1 q2 wk 40 r58,4` = building, queue length, head item and its progress %, rally point (`out` after it when the rally is more than 30 from home, `out failed` when fewer than 5 riflemen remain); `IDLE` = finished with an empty queue; `bld 60` = under construction, 60%.
...
5. `B` ends in `(2nd ba)` (ore ≥ 200, every barracks in `P` at `q3` or more) → `b ba <yard>`, once: two barracks, never three.
...
4. Rally out → the attack is on. Rifleman clusters in state `i`, `out` or not → `am <their A labels> <target>`. Never bring them home. If `P` marks the rally `out failed` (`H tr` under 5) the attack has failed: `ry <every barracks> <post>` and `am troopers <post>`.
