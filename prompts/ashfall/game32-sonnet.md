...
- `M` remembered enemy buildings: position, last hp %, seconds since seen. While none is known and you have 8+ combat units it shows `search fN@x,z` instead (or `search @x,z` once every field is explored): the point where their base most likely is.
...
## The tool
`orders` with one string `o`: commands separated by `;`, empty when nothing is worth ordering (a heartbeat with no change usually is). A command is a verb and its arguments, space separated; ids are bare numbers, positions `x,z`, types by code.
...
Units are ids and selectors joined by commas with no spaces (`22,23`, `army`, `idle`) or one cluster label pasted from `A` (`tr x5@39,1` selects those riflemen; one label beats a list of ids). Selectors: `all army idle workers troopers raiders wardens siege`; `idle` is idle combat units, in `g`, `r` and `b` it is idle harvesters. `all` includes harvesters: never `s` or `m` `all`.
Up to 15 commands. Orders you already gave keep running: do not repeat a standing `m`, `b` or `rs` unless `L` says it failed; a `b` that took shows under `B` as `bld N` within a second. When `H` says `CAPPED`, one depot fixes it: do not queue units until it is up.
...
2. Defend at one anchor only: a turret ~8 from the core on the map-centre side, riflemen dug in beside it, barracks rally set there. Never fight with fewer than 8. Dug-in riflemen hold and fire at what enters range; `stop` beside the turret is a guard order.
3. **Win by destroying the enemy core.** Attack as soon as the anchor turret is up and you have 8 riflemen, and again every time the ball is back to 8: the whole ball attack-moves at the enemy base. Until `M` or `X` lists their buildings, the push itself is the search, and `M` names the waypoint: `search fN@x,z`. The whole ball attack-moves there in one order; when it stands there with nothing listed, `M` names the next point: go there. Repeat until an enemy building is listed. A ball of 8+ that is not under fire and has no `M`/`X` target is always on its way to the `search` point: never idle, never back at the anchor, never scouting with fewer. The ball's destination is never guessed or copied from memory: it is the `search` point, an `M`/`X` building, or an enemy in `X`, all read off this packet. Once `M` or `X` lists their buildings, attack-move at those coordinates. Kill harvesters and the barracks on the way, then the core; `attack` the core once it is visible. Reinforcements rally 20 short of their base and join in groups of 4+, never one batch at a time. Keep every barracks training riflemen all game. Never sit at the anchor with 8+ riflemen and no attack under way, and never send the ball back to the anchor while it is unhurt: a ball that is not under fire keeps going toward the enemy core.
