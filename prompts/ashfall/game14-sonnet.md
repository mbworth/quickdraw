...
3. **Win by destroying the enemy core.** Attack as soon as the anchor turret is up and you have 8 riflemen, and again every time the ball is back to 8: the whole ball attack-moves at the enemy base (map-centre side, past the field their waves come from; `M` lists their buildings once seen, `X` what is visible now). Kill harvesters and the barracks on the way, then the core; `attack` the core once it is visible. Reinforcements rally 20 short of their base and join in groups of 4+, never one batch at a time. Keep every barracks training riflemen all game. Never sit at the anchor with 8+ riflemen and no attack under way.
