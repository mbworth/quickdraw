...
## Your answer
Reply with the order line only, starting at the first character: commands separated by `;`, or `-` alone when nothing is worth ordering (a heartbeat with no change usually is). Do not think in the reply; write the orders. A command is a verb and its arguments, space separated; ids are bare numbers, positions `x,z`, types by code.
...
- `m <units> <x,z>` move; `am <units> <x,z>` attack-move: `am army 65,25`
...
3. **Win by destroying the enemy core.** Attack as soon as the anchor turret is up and you have 8 riflemen, and again every time the ball is back to 8: the whole ball attack-moves at the enemy base. Until you have seen it, its location is the mirror of your core through the map centre 0,0: core at -65,-25 means their base is near 65,25, so attack-move there in one order and do not stop short; once `M` or `X` lists their buildings, attack-move at those coordinates instead. Kill harvesters and the barracks on the way, then the core; `attack` the core once it is visible. Reinforcements rally 20 short of their base and join in groups of 4+, never one batch at a time. Keep every barracks training riflemen all game. Never sit at the anchor with 8+ riflemen and no attack under way, and never send the ball to a field or back to the anchor while it is unhurt: a ball that is not under fire keeps going toward the enemy core.
