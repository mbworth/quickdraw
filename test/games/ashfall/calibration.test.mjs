// Calibrates the chars-per-token divisor from messages.countTokens over every fixture packet. Needs a key.
// Writes calibration.json.new next to the committed file and checks the committed value still holds (±10%).
...
const COMMITTED = path.resolve(path.dirname(new URL(import.meta.url).pathname), '../../../src/games/ashfall/calibration.json');
...
test('the committed divisor exists, is plausible, and is what the adapter uses', async () => {
  ...
  const mod = await import('../../../src/games/ashfall/index.mjs');
