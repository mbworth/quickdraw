...
- A10. `expand(cmds, state) → {cmds, dropped:[{cmd, reason}]}`: client-side sugar becomes wire cmds (counts fan out, clamps applied). Every trim is a `dropped` with a reason so it reaches `lastOrders`.
...
- M2. Thinking: `thinking:{type:'adaptive'}` + `output_config:{effort:'low'}` by default; `--thinking off` sends `{type:'disabled'}` on models that accept it; Haiku 4.5 gets neither `thinking` nor `effort`. Experiment variable.
